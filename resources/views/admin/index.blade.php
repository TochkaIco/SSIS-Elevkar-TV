<x-layout title="Admin">
    <div>
        <header class="py-8 md:py-12">
            ...
            <p class="text-muted-foreground text-sm mt-2">Share Elevkår's events and plans.</p>
            <x-card
                x-data
                is="button"
                    ...
                    <x-card href="{{ route('event.show', $event) }}">
                        @if($event->image_path)
                            <div class="mb-4 -mx-4 -mt-4 rounded-t-lg overflow-hidden">
                                <img src="{{ asset('storage/' . $event->image_path) }}" alt="image" class="w-full h-auto max-h-60 object-cover mb-2">
                            </div>
                        @endif
                        ...
                        <div class="mt-2">
                            <x-event.status-label status="{{ $event->status->label() }}">
                                {{ $event->status->label() }}
